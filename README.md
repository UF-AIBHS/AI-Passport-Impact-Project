...
# book.yml has no custom-CSS hook, so the Palatino font, link color, and
# default (light) theme are patched onto the built site directly — run
# after every build, before previewing.
./scripts/fix-font.sh
./scripts/fix-links.sh
./scripts/fix-default-theme.sh
```
