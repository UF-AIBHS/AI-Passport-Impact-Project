...
A [Digital Garden](https://github.com/oleeskild/Obsidian-Digital-Garden) publishing mini-projects and open datasets for the [UF AI Passport for Health Science](https://prismap.medicine.ufl.edu/research/ai-passport/), alongside the [ML4LLM book](https://github.com/saurabhr/ML4LLM_book).

Deployed to GitHub Pages at https://uf-aibhs.github.io/ai.passport.projects/ via [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) on every push to `main`.

## Publishing notes from Obsidian
1. Install the [Digital Garden plugin](https://github.com/oleeskild/Obsidian-Digital-Garden) in your Obsidian vault.
2. In the plugin settings, point it at this GitHub repo (`UF-AIBHS/ai.passport.projects`) with a [personal access token](https://docs.forestry.md/getting-started/quickstart#3-connect-obsidian-to-github) that has `contents: write` on it.
3. Mark notes with `dg-publish: true` in frontmatter and publish — the plugin commits them to `src/site/notes/`, which triggers the deploy workflow.
...
npm run dev
```
Runs at `http://localhost:8080` with no path prefix (the `/ai.passport.projects/` prefix is only applied in the GitHub Pages build).

---
## Docs
Plugin docs are available at [docs.forestry.md](https://docs.forestry.md/)
